...
public class MapFragment extends MvpAppCompatFragment implements MapView, OnMapReadyCallback, GoogleMap.OnMapClickListener {
    ...
    @BindView(R.id.action_forecast)
    FloatingActionButton mActionForecast;
    ...
    @Override
    public void openForecastDetailScreen() {
        if (getActivity() != null)
    ...
    @Override
    public void drawMarker(LatLng coords) {
        if (mMap != null) {
            mMarker = mMap.addMarker(new MarkerOptions().position(coords).
                    icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_RED)).draggable(true));
